...
export function AlunoDialog({ aluno }: { aluno: any }) {
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-xl px-6 py-5">
        ...
        <div className="space-y-5 text-sm text-muted-foreground">
          ...
          <div className="grid grid-cols-2 gap-5">
            <InfoItem icon={TrendingUp} label="CRA" value={aluno.cra} />
            <InfoItem
              icon={BookOpen}
              label="Matérias Cursadas"
              value={aluno.materiasCursadas}
            />
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="w-full">
            ...
            <ScrollArea className="h-[120px] w-[25rem] pr-2">
              {aluno.vagasInscritas.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-foreground space-y-1">
                  {aluno.vagasInscritas.map((vaga: any) => (
                    <li key={vaga.vagaId}>{vaga.title}</li>
                  ))}
                </ul>
              ) : (
                <p className="text-muted-foreground italic">
                  Nenhuma vaga inscrita.
                </p>
              )}
            </ScrollArea>
          </div>
